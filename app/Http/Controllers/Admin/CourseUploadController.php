...
class CourseUploadController extends Controller
{
    ...
    public function importCourseData(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Course data successfully imported.');
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    //     try {
    //         // Find the first non-header row with course/teacher info
    //         $headerRow = $rows[0][0] ?? [];
    //         $firstDataRow = null;
    //         foreach ($rows[0] as $index => $row) {
    //             if ($index == 0)
    ...
    //         // Check if TeacherWiseCourse already exists
    //         $exists = TeacherWiseCourse::where([
    //             'teacher_id' => $teacher->id,
    //             'course_id' => $course->id,
    //             'department_id' => $request->department_id,
    ...
    //         // Process all student rows (skip header)
    //         foreach ($rows[0] as $index => $row) {
    //             if ($index == 0)
    //                 continue;
    //             $student_name = $row[4] ?? null;
